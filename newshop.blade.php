              ...
              <div class="border-b border-gray-200 py-6">
                ...
                <div class="pt-6" id="filter-section-2">
                  <div class="space-y-4">
                    ...
                    <div class="flex items-center">
                      <input id="filter-size-0" name="brands[]" value="{{ $brand->id }}" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
  ...
  <section id="products-container" class="w-fit mx-auto grid grid-cols-1 xl:grid-cols-3 md:grid-cols-2 justify-items-center justify-center gap-y-20 gap-x-14 mt-10 mb-5">
    ...
    @endif
        ...
        <div class="px-4 py-3 w-72">
          <span class="text-gray-400 mr-3 uppercase text-xs font-Roboto-condensed">{{ $brand->name }}</span>
          ...
          <div class="flex items-center">
            ...
            @else
              @if($product->quantite > 0)
                <a href="{{ Route('login') }}" class="ml-auto add-products-to-wishlist" >
                  <div class="HeartAnimation"></div>
                </a>
              @endif
            @endif
    ...
    document.addEventListener('click', function(event) {
      ...
      sortOptions.forEach(option => {
        option.addEventListener('click', function(event) {
          ...
          updateProducts(sortType, selectedPrices, selectedCategories, searchTerm, selectedBrands);
        });
      });
      ...
      priceFilters.forEach(filter => {
        filter.addEventListener('change', function() {
          ...
          const sortType = sortOption ? sortOption.dataset.sort : '';
          const searchTerm = searchInput.value.trim();
          updateProducts(sortType, [selectedPrice], selectedCategories, searchTerm, selectedBrands);
        });
      });
